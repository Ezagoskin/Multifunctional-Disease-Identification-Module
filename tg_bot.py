...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("You can send here an optical coherence tomography image or path to "
                                    "one and get a prediction of the image class (NORMAL, CNV, DME or DRUSEN).")
...
async def classify_by_path(update: Update, context: ContextTypes.DEFAULT_TYPE):
    img_path = update.message.text
    try:
        im = cv2.imread(img_path)
        res = predict(im)
    except:
        res = 'Invalid Path or Image Format'
